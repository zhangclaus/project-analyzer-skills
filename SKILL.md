...
1. **识别核心流程**（用 Q1 的集成模型和模块分析）:
   - 找出项目中所有独立的端到端工作流
   - 判断标准: 有独立入口、独立目的、独立数据流
   - 大多数项目 1 条主流; 系统工具/平台项目可能有 2-3 条
   - 例: CodeGraph 有两条 — 入库（CLI index → 解析 → 存储）和检索（MCP 请求 → 图遍历 → 上下文组装）
2. **逐条追踪**每条流程:
   - 从入口开始，追踪调用链直到完成
   - 记录每步: 发生了什么、在哪个模块、分支、副作用
   - 识别每条流程的关键路径（happy path）
3. **输出**: 每条流程一个 Mermaid 图 + 步骤说明

**找流程入口:** CLI 命令 / API 端点 / MCP 工具 / hook 注册 / README 主用例 / 被引用最多的函数。
